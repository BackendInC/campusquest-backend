...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
...
def update_post(post_id: int, post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    #get the post by id
...
def delete_post(post_id: int, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    #get the post by id
    post = db.query(models.Posts).filter(models.Posts.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
def toggle_like(post_id: int, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    ...
    like = db.query(models.PostLikes).filter(models.PostLikes.user_id == current_user, models.PostLikes.post_id == post_id).first()
...
def count_post_likes(post_id: int, db: Session = Depends(get_db)):
    ...
    #count the number of likes for a post
    count = db.query(models.PostLikes).filter(models.PostLikes.post_id == post_id).count()
    return count

#check if a user has liked a post
@router.get("/posts/{post_id}/like", response_model=bool)
def check_user_like(post_id: int, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    #check if post exists
    post = db.query(models.Posts).filter(models.Posts.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
def read_post_likedby(post_id: int, db: Session = Depends(get_db)):
    ...
    users = db.query(models.User).join(models.PostLikes).filter(models.PostLikes.post_id == post_id).all()
    return users

#comment on a post
@router.post("/posts/{post_id}/comment", response_model=schemas.PostCommentResponse)
def create_comment(post_id: int, comment: schemas.PostCommentCreate, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    #check if post exists
    post = db.query(models.Posts).filter(models.Posts.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
def read_post_comments(post_id: int, db: Session = Depends(get_db)):
    ...
    comments = db.query(models.PostComments).filter(models.PostComments.post_id == post_id).all()
    return comments
...
def read_comment(comment_id: int, db: Session = Depends(get_db)):
    ...
    comment = db.query(models.PostComments).filter(models.PostComments.id == comment_id).first()
    if not comment:
...
def update_comment(comment_id: int, comment: schemas.PostCommentCreate, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    #get the comment by id
    comment_db = db.query(models.PostComments).filter(models.PostComments.id == comment_id).first()
    if not comment_db:
        raise HTTPException(status_code=404, detail="Comment not found")
...
def delete_comment(comment_id: int, db: Session = Depends(get_db), current_user: int = Depends(auth.decode_jwt)):
    #get the comment by id
    comment = db.query(models.PostComments).filter(models.PostComments.id == comment_id).first()
    if not comment:
        raise HTTPException(status_code=404, detail="Comment not found")
